 ...
 * 		global_work_size[0] = minimum multiple of WAVEFRONT larger that inputVolume = inputDepth*inputHeight*inputWidth
 ...
 */
 ...
 __kernel void ZeroPad (__read_only __global float* input,
...
{	
	...
	if (iInput < inputVolume) // this is important because of how local_work_size is set (more efficient)
	{
		const uint iSlice = (iInput % inputVolume) / inputArea; // find index of channel within an input volume
		const uint iRow = (iInput % inputArea) / inputWidth; // find index of row within an input channel
		const uint iOutput =  zerosPerSlice*iSlice + topRowsOfZeros + padding * (2*iRow + 1) + iInput;

		paddedInput[iOutput] = input[iInput];
	}
